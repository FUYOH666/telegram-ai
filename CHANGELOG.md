...
- **Убрано повторяющееся представление бота** - бот больше не повторяет информацию о себе в каждом сообщении
  - Добавлен флаг `introduced` в контекст пользователя для отслеживания представления
  - Бот представляется только один раз в начале диалога (первое сообщение или этап GREETING)
  - После представления бот общается естественно, без повторения информации о себе
  - Обновлен системный промпт с инструкцией о представлении только один раз
  - Обновлен промпт для этапа GREETING с условием представления
  - Добавлены примеры ответов для первого знакомства и для уже знакомых пользователей
  - Диалог теперь выглядит более человечным и менее роботизированным

## [0.6.7] - 2025-11-06

### Fixed

- **Исправлена ссылка на GitHub аккаунт** - теперь бот использует правильный GitHub аккаунт
  - Добавлена правильная ссылка на GitHub: https://github.com/FUYOH666 в системный промпт
  - Добавлен раздел "Контакты и ресурсы" в базу знаний с правильным GitHub аккаунтом
  - Теперь бот будет корректно указывать GitHub аккаунт при запросах пользователей
