...
- **Умная система защиты от блокировок Telegram** - комплексная система rate limiting
  - Глобальный rate limiter на уровне аккаунта (25 сообщений/минуту, 500/час)
  ...
  - Умные лимиты по типам чатов (личные: 20/мин, группы: 10/мин, каналы: 5/мин)
...
- Расширена документация README.md с описанием новых функций защиты

### Technical

- Новые модели БД: `GlobalRateLimit`, `FloodWaitHistory`
- Новые классы: `GlobalRateLimiter` в `rate_limiter.py`
- Обновлены: `client.py` (интеграция GlobalRateLimiter, safe_reply), `config.py` (новые конфигурации), `memory.py` (новые модели), `main.py` (health check)
